...
Script 07 of the AKOYA analysis series.

WHAT THIS FIXES FROM SCRIPT 06
    The script 06 nearest-neighbour mixed models used RAW DISTANCE IN MICRONS as
    the outcome. Distances inside a focus are bounded by that focus's size, and
    treated foci have a median radius of 132 um against 212 um untreated. A
    small focus plus its 150 um cuff cannot geometrically produce a 250 um
    nearest-neighbour distance. Coefficients such as -121 um (IDO1+ to Tregs)
    and -92 um (IDO1- to Tregs) therefore largely restate that treated foci are
    ...
    The per-animal delta values in script 06 table 54 remain valid, because they
    subtract a within-focus permutation null.

    NOTE ON DIRECTION: treated animals have far fewer Tregs (6, 26, 2) than
    untreated (59, 23, 393), and scarcity should push treated distances UP.
    They went down instead, which is the opposite of the expected bias. Some
    real effect may sit underneath the confound, which is why this script
    re-tests it three independent ways rather than discarding it.

WHAT THIS SCRIPT DOES
    A  Size confound diagnostic. Observed nearest-neighbour distance against
       focus equivalent radius, per structure. Makes the problem visible before
       any correction is applied.
    B  Three independent distance outcomes, each modelled separately:
         1. NORMALISED  distance divided by that focus's equivalent radius
         2. DELTA       distance minus that focus's own permutation null median
         3. RAW + COVARIATE  raw distance with focus radius as a covariate
       (3) is a DIAGNOSTIC ONLY. Radius is nearly collinear with arm, so its
       standard errors are inflated and it cannot serve as a primary test. An
       effect that survives (1) and (2) is real; one that appears only in raw
       distance is size.
    C  Benjamini-Hochberg correction within declared families, so the multiple
       testing position is stated rather than left implicit. Nothing in script
       06 survived BH; that needs to be visible, not buried.
    D  Polarisation at the 60th percentile instead of the 75th, which widens the
       exclusively-polarised classes and should recover mixing rows for 43106
       and 43118 (both produced none at 75).
    E  The iNOS / Arginase-1 co-expression ratio as a proper figure with the
       full threshold sweep. At the 75th percentile this separated the arms
       perfectly (D1MT 1.12, 1.36, 1.49; Untreated 1.52, 1.62, 1.73) with no
       overlap. Because it is a within-section correlation between two markers
       with fixed marginals, global intensity differences between the two scans
       largely cancel, making it more robust to the slide confound than most of
       the panel.
...
PREREQUISITE
    Rerun script 04 with FOCUS_FOLD_OVER_BACKGROUND = 6.0 first. As of the last
    run every treated animal still had exactly one focus, so 43118 contributed
    no IDO1+ macrophage row and several comparisons rest on n = 2.
...
BH_ALPHA = 0.10                    # reported at 0.05 and 0.10
...
banner("AKOYA SIZE-CORRECTED DISTANCE MODELS AND POLARISATION")
...
print("    used as a model outcome. Valid outcomes here: distance divided by")
print("    focus radius, and distance minus that focus's permutation null.")
print("    Raw distance with radius as a covariate is a diagnostic only,")
print("    because radius is nearly collinear with arm.")
...
if os.path.exists(FOCI_TABLE):
    ...
    for _, r in ft.iterrows():
        radius_of[(r["sample_id"], int(r[idcol]))] = float(r["equiv_radius_um"])
    print(f"\n    loaded geometry for {len(radius_of)} structures")
else:
...
tstruct = {s: int(cells[s].loc[cells[s][STRUCT_COL] > 0, STRUCT_COL].nunique())
           for s in SAMPLE_ORDER if COND_OF[s] == "D1MT"}
if tstruct and max(tstruct.values()) <= 1:
    print("    WARNING: every treated animal still has ONE structure.")
    print("    Rerun script 04 with FOCUS_FOLD_OVER_BACKGROUND = 6.0.")
else:
    print(f"    treated structures per animal: {tstruct}")
...
def focus_radius(sid, k, g):
    r = radius_of.get((sid, int(k)))
    ...
    return float(0.5 * np.hypot(x.max() - x.min(), y.max() - y.min()))
    ...
    for anchor in NN_ANCHORS:
        ...
        for target in NN_TARGETS:
            ...
            struct_rows.append({
                ...
                "normalised_median": obs / rad if (rad and np.isfinite(rad)) else np.nan,
            ...
            percell_frames.append(pd.DataFrame({
                ...
                "distance_um": take,
                "focus_radius_um": rad,
                "distance_norm": take / rad if (rad and np.isfinite(rad)) else np.nan,
...
if len(nn_struct):
    ...
    sub("Correlation of raw distance with focus radius, per pair")
    corr_rows = []
    for (a, t), g in nn_struct.groupby(["anchor", "target"]):
        gg = g.dropna(subset=["observed_median_um", "focus_radius_um"])
        if len(gg) < 4:
            continue
        r_raw = float(np.corrcoef(gg["observed_median_um"],
                                  gg["focus_radius_um"])[0, 1])
        gn = gg.dropna(subset=["normalised_median"])
        r_norm = (float(np.corrcoef(gn["normalised_median"],
                                    gn["focus_radius_um"])[0, 1])
                  if len(gn) >= 4 else np.nan)
        gd = gg.dropna(subset=["delta_um"])
        r_delta = (float(np.corrcoef(gd["delta_um"], gd["focus_radius_um"])[0, 1])
                   if len(gd) >= 4 else np.nan)
        corr_rows.append({"anchor": a, "target": t, "n_structures": len(gg),
                          "corr_raw_vs_radius": r_raw,
                          "corr_normalised_vs_radius": r_norm,
                          "corr_delta_vs_radius": r_delta})
        print(f"    {a[:22]:<24} -> {t:<16} raw r={r_raw:>+6.2f}   "
              f"norm r={r_norm:>+6.2f}   delta r={r_delta:>+6.2f}")
    corrs = pd.DataFrame(corr_rows)
    write_csv(corrs, "61_distance_radius_correlation.csv")
    print("\n    A large positive raw correlation confirms the confound. The")
    print("    normalised and delta correlations should be much closer to zero;")
    print("    if they are not, that outcome has not removed the size effect.")

    # ---- F47 the confound --------------------------------------------------
    fig, axes = plt.subplots(1, 3, figsize=(42, 13))
    for ax, col, ttl in zip(
            axes,
            ["observed_median_um", "normalised_median", "delta_um"],
            ["Raw distance (INVALID outcome)", "Distance / focus radius",
             "Distance minus focus null"]):
        for s in SAMPLE_ORDER:
        ...
        ax.set_ylabel(ttl, fontsize=FONT_SIZE_BASE - 12)
    ...
    fig.suptitle("Why raw distance cannot be compared between arms\n"
                 ...
                 "smaller regardless of biology", y=1.04,
                 fontsize=FONT_SIZE_TITLE - 6)
    save_fig(fig, "F47_size_confound_diagnostic")


# %% Cell 5 - B/C: three distance outcomes with BH correction
# =============================================================================

banner("B/C - DISTANCE MODELS ON THREE OUTCOMES, WITH BH CORRECTION")

model_rows = []
if RUN_MIXED_MODELS and HAVE_SM and len(percell):
    for (a, t), g in percell.groupby(["anchor", "target"]):
        for outcome, fam, cov, note in [
            ("distance_norm", "distance_normalised", None,
             "distance / focus radius"),
            ("distance_delta_um", "distance_delta", None,
             "distance minus focus permutation null"),
            ("distance_um", "distance_raw_diagnostic", "focus_radius_um",
             "DIAGNOSTIC ONLY: radius nearly collinear with arm"),
        ]:
...
# radial models, valid as-is, refitted here so everything carries q-values
if RUN_MIXED_MODELS and HAVE_SM:
    for p in KEY_PHENOTYPES:
        ...
        r = fit_mixed(dd, "radial_pos", f"radial: {p}",
                      note="CIRCULAR: population defines the foci" if circ else "")
        if r:
...
if len(models):
    ...
    for fam in ["radial_position", "distance_normalised", "distance_delta",
                "distance_raw_diagnostic"]:
        g = models.loc[models["family"] == fam].sort_values("p_value")
        if not len(g):
            continue
        flag = ("   [DIAGNOSTIC ONLY]" if fam == "distance_raw_diagnostic" else "")
    ...
    sub("Which effects survive size correction")
    for (a, t), g in models.loc[models["family"].isin(
            ["distance_normalised", "distance_delta"])].groupby(["anchor", "target"]):
        if len(g) < 2:
            continue
        norm = g.loc[g["family"] == "distance_normalised"]
        delt = g.loc[g["family"] == "distance_delta"]
        if not len(norm) or not len(delt):
            continue
        pn, pd_ = float(norm["p_value"].iloc[0]), float(delt["p_value"].iloc[0])
        cn, cd = float(norm["coef_D1MT_vs_ref"].iloc[0]), float(delt["coef_D1MT_vs_ref"].iloc[0])
        same_dir = np.sign(cn) == np.sign(cd)
        verdict = ("SURVIVES both" if (pn < 0.05 and pd_ < 0.05 and same_dir)
                   else "partial" if (pn < 0.05 or pd_ < 0.05)
                   else "does not survive")
        print(f"    {a[:22]:<24} -> {t:<16} norm p={pn:.4f}  delta p={pd_:.4f}  "
              f"same direction={same_dir}   -> {verdict}")

    sub("Multiple testing position")
    n_p05 = int(models["sig_p05"].sum())
    n_q10 = int(models["sig_q10"].sum())
    print(f"    {n_p05} of {len(models)} tests have p < 0.05")
    print(f"    {n_q10} of {len(models)} survive BH at q < {BH_ALPHA}")
    if n_q10 == 0:
    ...
    fams = [f for f in ["radial_position", "distance_normalised",
                        "distance_delta"] if (models["family"] == f).any()]
    ...
    for ax, fam in zip(axes[0], fams):
        ...
        ax.set_yticks(yy)
        labels = [(r["analysis"][:38] + (" [circ]" if r.get("circular", False) else ""))
                  for _, r in g.iterrows()]
        ax.set_yticklabels(labels, fontsize=FONT_SIZE_TICK - 14)
        ax.set_xlabel("D1MT effect vs Untreated (95% CI)",
                      fontsize=FONT_SIZE_BASE - 10)
        ax.set_title(fam.replace("_", " "), fontsize=FONT_SIZE_TITLE - 10)
    ...
    fig.suptitle("Treatment effects by outcome family, with BH correction\n"
                 "Raw-distance models are excluded: that outcome is bounded by "
                 "focus size", y=1.03, fontsize=FONT_SIZE_TITLE - 6)
...
for s in SAMPLE_ORDER:
    ...
    for k, g in d.loc[(d[STRUCT_COL] > 0)
        ...
        def mixing(am, bm):
            ...
            return float(bm[idxs[rows]].mean()) if len(rows) else np.nan
...
if len(pol):
    ...
    if clean:
        print(f"\n    Complete separation between arms at percentiles: {clean}")
        print("    Because this is a within-section correlation between two")
        print("    markers with fixed marginals, global intensity differences")
        print("    between the two scans largely cancel. That makes it more")
        print("    robust to the slide confound than most of the panel.")
    ...
    fig.suptitle("iNOS and Arginase-1 co-expression in macrophages\n"
                 "Every animal above 1, so a strict M1/M2 dichotomy does not "
                 "hold in either arm", y=1.04, fontsize=FONT_SIZE_TITLE - 8)
...
if len(nn_struct):
    per_animal_norm = (nn_struct.groupby(["sample_id", "condition", "anchor", "target"])
                       ["normalised_median"].median().reset_index())
    consistency(per_animal_norm, ["anchor", "target"], "normalised_median",
                "nn_normalised")
    per_animal_delta = (nn_struct.groupby(["sample_id", "condition", "anchor", "target"])
                        ["delta_um"].median().reset_index())
    consistency(per_animal_delta, ["anchor", "target"], "delta_um", "nn_delta")
if len(pol):
    p0 = pol.loc[pol["percentile"] == POLARISATION_PRIMARY].copy()
    p0["phenotype"] = "iNOS_Arg1_coexpression"
...
if len(cons):
    ...
    if not len(sep):
        ...
        print("    the minimum p a rank test can return (0.10, two-sided).")

banner("SUMMARY")
print(f"Structures with geometry : {len(nn_struct)} pair-rows")
print(f"Models fitted            : {len(models)}")
if len(models):
    print(f"  p < 0.05               : {int(models['sig_p05'].sum())}")
    print(f"  q < {BH_ALPHA} (BH)          : {int(models['sig_q10'].sum())}")
print(f"Polarisation rows        : {len(pol)}")
print(f"Mixing rows              : {len(mix)}")

sub("Read in this order")
print("  1. F47 / table 61 : the size confound, and whether the corrections")
print("     removed it. Check that normalised and delta correlations with")
print("     radius are near zero.")
print("  2. F48 / table 62 : models by family with BH q-values. The distance")
print("     conclusion is whatever survives BOTH normalised and delta.")
print("  3. F49 / table 64 : co-expression ratio. Currently the cleanest arm")
print("     separation in the whole analysis.")
print("  4. F50 / table 65 : polarisation mixing, now at the 60th percentile.")
print("  5. Table 66       : complete separation between arms.")

sub("What is claimable after this run")
print("  - Burden: near-zero granuloma-density tissue in treated animals under")
print("    a common definition. Strongest and simplest result.")
print("  - Architecture: lymphocytes core-ward in treated foci, rim-ward in")
print("    untreated lesions. Coherent across populations, with neutrophils")
print("    inverting as necrosis predicts. A pattern argument, not a single p.")
print("  - Polarisation: iNOS and Arginase-1 co-expressed above chance in every")
print("    animal, more so in untreated. Within-section, so batch-robust.")
print("  - NOT claimable: anything from raw distances in microns, and anything")
print("    from 3-hydroxykynurenine.")
